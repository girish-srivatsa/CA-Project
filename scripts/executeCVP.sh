...
tracedir=~/car3s/qualcomm_traces/intensive_traces/
	...
	then
		...
		do
			if [ $i -eq `expr $numofpref - 1` ]
                        then
				./run_champsim.sh $branchPredictor-${l1ipref[i]}-${l1dpref[i]}-${l2cpref[i]}-${llcpref[i]}-${itlb_pref[i]}-${dtlb_pref[i]}-${stlb_pref[i]}-$cache_repl-$tlb_repl-1core $numwarmup $numsim $tracedir $trace -cvp_trace
			else
				./run_champsim.sh $branchPredictor-${l1ipref[i]}-${l1dpref[i]}-${l2cpref[i]}-${llcpref[i]}-${itlb_pref[i]}-${dtlb_pref[i]}-${stlb_pref[i]}-$cache_repl-$tlb_repl-1core $numwarmup $numsim $tracedir $trace -cvp_trace &
			fi
		done
