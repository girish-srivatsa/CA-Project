  ...
  if (bank_request[op_channel][op_rank][op_bank].cycle_available <=
    ...
    if (dbus_cycle_available[op_channel] <= current_core_cycle[op_cpu]) {
      ...
      if (queue->is_WQ) {
        ...
        if (queue->entry[request_index].type == LOAD_TRANSLATION ||
        queue->entry[request_index].type == PREFETCH_TRANSLATION ||
        queue->entry[request_index].type == TRANSLATION_FROM_L1D)
